...
signInAnonymously(auth)
  .then(() => {
    ...
    callButton.onclick = async () => {
      ...
      // Generate a shareable link with the call ID
      const shareableLink = `https:/videocalltm.vercel.app/?callId=${callDoc.id}`;
      navigator.clipboard.writeText(shareableLink).then(() => {
        ...
        alert('Failed to copy link:', err);
         ...
         // Assuming this code is on the website that opens the shareable link
        window.onload = function() {
        const urlParams = new URLSearchParams(window.location.search);
        const callId = urlParams.get('callId');
        const callInput = document.getElementById('callInput');
        if (callId) {
          callInput.value = callId; // Auto-input the call ID
        }
        };
